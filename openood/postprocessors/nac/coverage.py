...
class Coverage:
    def __init__(self,
        ...
        self.init_variable(features, hyper)
...
class KMNC(Coverage):
    def init_variable(self, layer_features, hyper: Optional[Dict] = None):
        ...
        for (ln, layer_size) in self.layer_size_dict.items():
            ...
            # todo: 
            all_features = np.concatenate(layer_features[ln], axis=0)

            self.estimator_dict[ln] = Estimator(layer_size, all_features, M, O, self.device, ln)
    ...
    def assess(self, model, data_loader, index, spatial_func=None, **kwargs):
        model.eval()
        if spatial_func is None:
        ...
        with nethook.InstrumentedModel(model) as instr:
            ...
            for i, data in enumerate(tqdm(data_loader)):
                ...
                for j, ln in enumerate(self.layer_names):
                    ...
                    b_state = instr.retained_layer(ln)  # 0.1068, 0.0947,1.6035...
    ...
    def assess_ood(self, model, data_loader, index, spatial_func=None, progress=True, **kwargs):
        model.eval()
        if spatial_func is None:
            spatial_func = lambda s, m: s   # avgpool
        transform = lambda s, m: spatial_func(s, m).detach() if len(s.shape) > 2 else s.detach() # m: avg, for s 
        state_funcs = {ln: get_state_func(transform=transform, **self.hyper_dict[ln]) for ln in self.layer_names} # define four func 
        ...
        with nethook.InstrumentedModel(model) as instr:
            ...
            for i, data in enumerate(tqdm(data_loader, disable=not progress)):
                ...
                b_size = x.shape[0]
                p = model(x)
                correct_num, b_flags = acc(p, y)  # bool valse, for example [true, false, false]
                correct_flags = torch.ones_like(y).bool()  # we calculate cov for all samples 128
                b_layer_state = {}
                for j, ln in enumerate(self.layer_names):
                    # we use avg-pool for last layer except the vit.encoder.ln (which employs the first clss token)
                    pooling_method = "avg" if ln != "encoder.ln" else "first"
                    retain_graph = False if j == len(self.layer_names) - 1 else True  # True
                    b_state = instr.retained_layer(ln)  # avgpool 0.034
                    b_kl_grad = kl_grad(b_state, p, retain_graph=retain_graph)
                    ...
                    if ln =='avgpool':
                        features += b_layer_state[ln].cpu().detach().tolist()
                ...
                # TODO:
                _, feature_knn = model(x, return_feature=True)
                feature_normed = normalizer(feature_knn.data.cpu().numpy())
                D, _ = index.search(
                    feature_normed,
                    5,
                )
                ...
                kdis.append(kth_dist)

        scores = [5 * k + 1 * s for k, s in zip(kdis, scores)]#  ood 10, osr 5
        ...
        # features = scores.numpy()
        return scores, flags, preds, labels, features
    ...
    @staticmethod
    def load(path, name, layer_features, device=None, params=None, unpack=None, verbose=False):
        state = torch.load(os.path.join(path, name))
        if params is not None:
            # print("Update params:", params)
            state['hyper_dict'].update(params)
        Coverage = KMNC(layer_features, state['layer_size_dict'], device=device,
...
class Estimator(object):
    def __init__(self, neuron_num, all_features, M=1000, O=1, device=None, layer_name=None):
        ...
        # self.thresh = logspace(1e3, M).view(M, -1).repeat(1, neuron_num).to(self.device)  # nac 

        spatial_aggregated = np.mean(all_features, axis=(2, 3))  # shape: (698, 64)
        # Step 2: KDE 
        thresh = []
        for i in range(neuron_num): 
            kde = KernelDensity(kernel='gaussian', bandwidth=1).fit(spatial_aggregated[:, i:i+1])
            # bandwidth, ood set 1 , ors set 0.01
            samples = np.linspace(spatial_aggregated[:, i].min(), spatial_aggregated[:, i].max(), M)
            log_density = kde.score_samples(samples[:, None])
            cdf = np.cumsum(np.exp(log_density))  # CDF
            cdf /= cdf[-1]
            bin_edges = np.interp(np.linspace(0, 1, M), cdf, samples) 
            thresh.append(bin_edges)
        thresh = np.array(thresh).T  # shape: [M, neuron_num]
        self.thresh = torch.from_numpy(thresh).float().to(self.device)
        
        self.t_act = torch.zeros(M - 1, neuron_num).to(self.device)  # current activations under each thresh
        self.n_coverage = None

        # self.visualize_threshold_comparison(all_features, neuron_indices=[0, 1, 2, 3])

        if self.layer_name:
            save_path = f'threshold_comparison_{self.layer_name}.png'
        else:
            save_path = 'threshold_comparison.png'
            
        if not os.path.exists(save_path):
            self.visualize_threshold_comparison(all_features, save_path=save_path)
    ...
    def update(self, states):
        ...
        with torch.no_grad():
            ...
            b_act = b_act.sum(dim=0)  # [num_t, num_n]   # Indicates the number of activated samples for each threshold interval.
            # print(states.shape[0], b_act.sum(0)[:3])

            self.t_act += b_act  # current activation times under each interval
            # alpha = 0.1
            # self.t_act = alpha * self.t_act + (1 - alpha) * b_act

    def get_score(self, method="avg"):
        t_score = torch.min(self.t_act / self.O, torch.ones_like(self.t_act))  # [num_t, num_n]
        coverage = (t_score.sum(dim=0)) / self.M  # [num_n] Represents the average coverage of each neuron
        if method == "norm2":
    ...
    def ood_test(self, states, method="avg"):
        ...
        b_act = (states.unsqueeze(1) >= self.thresh[:self.M - 1].unsqueeze(0)) & \
                (states.unsqueeze(1) < self.thresh[1:self.M].unsqueeze(0))
        scores = (b_act * self.n_coverage.unsqueeze(0)).sum(dim=1)  # [num_data, num_n]  Interaction between activation interval and coverage
        if method == "avg":
    ...
    def visualize_threshold_comparison(self, all_features, neuron_indices=None, save_path=None):

        spatial_aggregated = np.mean(all_features, axis=(2, 3))  # shape: (num_samples, channels)
        
        if neuron_indices is None:
            
            neuron_indices = list(range(min(4, spatial_aggregated.shape[1])))
        
        num_neurons = len(neuron_indices)
        fig, axes = plt.subplots(2, num_neurons, figsize=(5 * num_neurons, 8))
        ...
        for idx, neuron_idx in enumerate(neuron_indices):
            ...
            neuron_data = spatial_aggregated[:, neuron_idx]
            
            uniform_thresh = np.linspace(neuron_data.min(), neuron_data.max(), self.M)
            
            kde_thresh = self.thresh[:, neuron_idx].cpu().numpy()
            
            ax_top = axes[0, idx]
            ax_bottom = axes[1, idx]
            
            ax_top.hist(neuron_data, bins=50, alpha=0.7, density=True, color='lightblue', edgecolor='black')
            ax_top.set_title(f'Neuron {neuron_idx} - Distribution and Thresholds', fontsize=12)
            ax_top.set_ylabel('Density')
            
            kde = KernelDensity(kernel='gaussian', bandwidth=1).fit(neuron_data.reshape(-1, 1))
            samples = np.linspace(neuron_data.min(), neuron_data.max(), 1000)
            log_density = kde.score_samples(samples[:, None])
            density = np.exp(log_density)
            ax_top.plot(samples, density, 'b-', linewidth=2, label='KDE')

            for i, (uniform_t, kde_t) in enumerate(zip(uniform_thresh[::self.M//10], kde_thresh[::self.M//10])):
                ax_top.axvline(uniform_t, color='green', linestyle='--', alpha=0.6, linewidth=1)
                ax_top.axvline(kde_t, color='red', linestyle='-', alpha=0.6, linewidth=1)
            

            from matplotlib.lines import Line2D
            legend_elements = [
                Line2D([0], [0], color='green', linestyle='--', alpha=0.6, label='Uniform'),
                Line2D([0], [0], color='red', linestyle='-', alpha=0.6, label='KDE')
            ]
            ax_top.legend(handles=legend_elements)

            uniform_intervals = np.diff(uniform_thresh)
            kde_intervals = np.diff(kde_thresh)
            
            x_positions = np.arange(len(uniform_intervals))
            width = 0.35
            
            ax_bottom.bar(x_positions - width/2, uniform_intervals, width, 
                         label='Uniform', alpha=0.7, color='green')
            ax_bottom.bar(x_positions + width/2, kde_intervals, width, 
                         label='KDE', alpha=0.7, color='red')
            
            ax_bottom.set_title(f'Neuron {neuron_idx} - Interval Comparison', fontsize=12)
            ax_bottom.set_xlabel('Interval Index')
            ax_bottom.set_ylabel('Interval Width')
            ax_bottom.legend()
