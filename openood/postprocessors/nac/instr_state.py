...
def kl_grad(b_state, outputs, temperature=1.0, retain_graph=False, **kwargs):
    ...
    targets = torch.ones_like(outputs) / num_classes   # uniform distribution
  ...
  # Calculate the gradient of Kullback Leibler (KL) divergence of a layer's output 
  # (c_state) with respect to the target distribution. This gradient reflects the degree of
  #  change in the layer's output relative to the target distribution and can be used to represent uncertainty.