...
# sample_number: 1000 sample_from: 10000 select: 1 feature_dim: 512  #resnet 512, lenet 120
...
def cosine_annealing(step, total_steps, lr_max, lr_min):
    ...
    # TODO：
def perturb_samples(samples, method="gaussian", strength=0.1):
    if method == "gaussian":
        noise = torch.randn_like(samples) * strength
        perturbed_samples = samples + noise

    elif method == "scaling":
        scale = 1 + (torch.randn(samples.size(0), 1, device=samples.device) * strength)
        perturbed_samples = samples * scale
        
    elif method == "directional":
        direction = torch.randn_like(samples)
        direction = direction / direction.norm(dim=-1, keepdim=True)
        perturbed_samples = samples + direction * strength
    else:
...
class VOSTrainer:
    def __init__(self, net, train_loader, config: Config):
        ...
        self.data_dict = torch.zeros(self.config['num_classes'],
                                     self.config['sample_number'],   # 1000
                                     self.config['feature_dim']).cuda()  # 512   [num_classes, sample_number, feature_dim]
        
    def train_epoch(self, epoch_idx):
        self.net.train()
        loss_avg = 0.0
        sample_number = self.config['sample_number']  # 1000
        num_classes = self.config['num_classes']  # 3 
        train_dataiter = iter(self.train_loader)
        eye_matrix = torch.eye(self.config['feature_dim'], device='cuda')   # 512
        ...
        for train_step in tqdm(range(1,
            ...
            x, output = self.net.forward(images, return_feature=True)  # batch, class / batch, 512

            sum_temp = 0
            for index in range(num_classes):
                sum_temp += self.number_dict[index]   # 0:num, 1:num, 2:num
            lr_reg_loss = torch.zeros(1).cuda()[0]
            if (sum_temp == num_classes * sample_number and epoch_idx < self.config['start_epoch']):
                ...
                for index in range(num_classes):
                    if index == 0:
                        ...
                        mean_embed_id = self.data_dict[index].mean(0).view(1, -1)  # 1, 512
                    else:
                ...
                temp_precision = torch.mm(X.t(), X) / len(X)   # 512, 512
                temp_precision += 0.0001 * eye_matrix
                for index in range(num_classes):
                    new_dis = MultivariateNormal(loc=mean_embed_id[index], covariance_matrix=temp_precision)
                    negative_samples = new_dis.rsample((self.config['sample_from'], ))  # 1000, 512
                    prob_density = new_dis.log_prob(negative_samples)
                    cur_samples, index_prob = torch.topk(-prob_density, self.config['select'])

                    selected_samples = negative_samples[index_prob]
                    perturbed_samples = perturb_samples(selected_samples, method="directional", strength=0.05)
                    if index == 0:
                        ood_samples = perturbed_samples
                    else:
                        ood_samples = torch.cat((ood_samples, perturbed_samples), 0)
